...
@section('content')
<div class="mb-6">
    <a href="{{ route('activities.index') }}" class="text-sm text-indigo-600 hover:text-indigo-900 flex items-center">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-white shadow-sm rounded-xl border border-slate-200 p-6">
            <h2 class="text-2xl font-bold text-slate-900">{{ $activity->title }}</h2>
                ...
                @if($activity->logs->count() > 0)
                    <div class="flow-root">
                        <ul role="list" class="-mb-8">
                            @foreach($activity->logs->sortByDesc('logged_at') as $log)
                            <li>
                                <div class="relative pb-8">
                                    ...
                                    <div class="relative flex space-x-3">
                                        <div>
                                            <span class="h-8 w-8 rounded-full flex items-center justify-center ring-8 ring-white {{ $log->status === 'done' ? 'bg-green-100' : 'bg-yellow-100' }}">
                                                @if($log->status === 'done')
                                        ...
                                        <div class="min-w-0 flex-1 pt-1.5 flex justify-between space-x-4">
                                            <div>
                                                <p class="text-sm text-slate-500">
                                                    Updated to <span class="font-medium text-slate-900 {{ $log->status === 'done' ? 'text-green-700' : 'text-yellow-700' }}">{{ ucfirst($log->status) }}</span> by 
                                                    <span class="font-medium text-slate-900">{{ $log->user->name }}</span>
                                                </p>
                                                @if($log->remark)
                                                    <p class="mt-2 text-sm text-slate-700 bg-slate-50 p-3 rounded-lg border border-slate-100">{{ $log->remark }}</p>
                                                @endif
                                            </div>
    ...
    <div class="lg:col-span-1">
        <div class="bg-white shadow-sm rounded-xl border border-slate-200 sticky top-6">
            ...
            <div class="px-6 py-6">
                <form action="{{ route('activity.logs.store', $activity) }}" method="POST">
                    @csrf
                    ...
                    <div class="space-y-5">
                        ...
                        <div>
                            ...
                            <div class="mt-1">
                                <textarea id="remark" name="remark" rows="3" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border border-slate-300 rounded-md px-3 py-2">{{ old('remark') }}</textarea>
                            </div>
                        </div>
                    </div>
